...
function calculateCGPAFromDB($db, $studentId, $sessionId) {
    try {
        // Get all semesters with marks for this student
        $query = "SELECT DISTINCT m.semester
                  FROM marks m
                  WHERE m.student_id = :student_id
                  AND m.session_id = :session_id
                  ORDER BY m.semester";
        ...
        foreach ($semesters as $semester) {
            $gpaData = calculateGPAFromDB($db, $studentId, $semester, $sessionId);
            
            if ($gpaData['total_credits'] > 0) {
                $semesterData[] = [
                    'gpa' => $gpaData['gpa'],
        ...
        return calculateCGPA($semesterData);
    } catch (PDOException $e) {
